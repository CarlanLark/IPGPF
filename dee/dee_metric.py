...
def agg_event_level_tpfpfn_stats(pred_records, gold_records, role_num):
    ...
    role_tpfpfn_stats = agg_event_role_tpfpfn_stats(
        pred_records, gold_records, role_num, polysemy_mat = None
    )
...
def measure_ins_prf(pred_record_mat_list, gold_record_mat_list,
                                event_type_roles_list,  
                                pred_event_types, gold_event_types,
                                pred_spans_token_tuple_list, gold_spans_token_tuple_list,
                                avg_type='micro',
    ...
    for pred_record_mat, gold_record_mat in zip(pred_record_mat_list, gold_record_mat_list):
        event_role_tpfpfn_stats = agg_ins_event_role_tpfpfn_stats(
            pred_record_mat, gold_record_mat, event_role_num_list
        ) #[(Event Index)[(Role Index)[TP, FP, FN]]]
        assert event_role_tpfpfn_stats != []
        prfs = {}
        total_tp, total_fp, total_fn = 0, 0, 0
        for event_idx, role_tpfpfn_stats in enumerate(event_role_tpfpfn_stats):
            tp, fp, fn = 0, 0, 0
            for tpfpfn_stats in role_tpfpfn_stats:
                tp += tpfpfn_stats[0]
                fp += tpfpfn_stats[1]
                fn += tpfpfn_stats[2]
            if fp == 0 and fn == 0:
                ...
                p, r, f =  get_prec_recall_f1(tp, fp, fn)
            prfs[event_type_roles_list[event_idx][0]] = {'Precision':p, 'Recall':r, 'Micro-F1':f}
            
            total_tp += tp
            total_fp += fp
            total_fn += fn
        if total_fp == 0 and total_fn == 0:
            total_p, total_r, total_f = 1.0, 1.0, 1.0
        else:
            total_p, total_r, total_f = get_prec_recall_f1(total_tp, total_fp, total_fn)
        prfs['Micro'] = {'Precision':total_p, 'Recall':total_r, 'Micro-F1':total_f}

        ins_prf_stats.append(prfs)
