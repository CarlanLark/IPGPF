...
from tools.ChildTuningOptimizer import ChildTuningAdamW
...
class BasePytorchTask(object):
    ...
    def __init__(self, setting, only_master_logging=False):
        ...
        #pre-filling
        self.pre_filling_config = {
            'use_pre_filling': None, 
            'ground_truth_pre_filling_ratio': None, 
        }
    ...
    def load_example_feature_dataset(self, load_example_func, convert_to_feature_func, convert_to_dataset_func,
        ...
        if os.path.exists(file_path):
            ...
            #print('#xxx#---debug mode')
            if self.setting.debug_mode:
                examples = load_example_func(file_path, only_inference = only_inference)[0:128]
            else:
                examples = load_example_func(file_path, only_inference = only_inference)
    ...
    def _load_data(self, load_example_func, convert_to_feature_func, convert_to_dataset_func,
                   load_train=True, load_dev=True, load_test=True, load_inference = False):
        ...
        if load_inference:
            ...
            self.inference_examples, self.inference_features, self.inference_dataset = self.load_example_feature_dataset(
                load_example_func, convert_to_feature_func, convert_to_dataset_func,
                file_name=self.setting.inference_file_name, only_inference=True, 
            )
        else:
            self.logging('Do not load inference portion')
    ...
    def reset_bert_optimizer(self):
        ...
        optimizer_grouped_parameters = [
            {
                'params': [p for n, p in model_named_parameters if (n not in no_decay) and ('ner_model.bert' in n)],
                'weight_decay_rate': 0.01, 
                'use_child_tune': True
            },
            {
                'params': [p for n, p in model_named_parameters if (n in no_decay) and ('ner_model.bert' in n)],
                'weight_decay_rate': 0.0, 
                'use_child_tune': True
            },
            {
                'params': [p for n, p in model_named_parameters if (n not in no_decay) and ('ner_model.bert' not in n)],
                'weight_decay_rate': 0.01, 
                'use_child_tune': False
            },
            {
                'params': [p for n, p in model_named_parameters if (n in no_decay) and ('ner_model.bert' not in n)],
                'weight_decay_rate': 0.0, 
                'use_child_tune': False
            }
        ]
        ...
        if self.setting.use_child_tune:
            optimizer = ChildTuningAdamW(optimizer_grouped_parameters, lr=self.setting.learning_rate, reserve_p=self.setting.reserve_p, mode=self.setting.child_tune_mode)
        else:
            optimizer = optim.AdamW(optimizer_grouped_parameters, lr=self.setting.learning_rate)
    ...
    def calculate_fisher(self):
        ...
        for name, params in model.named_parameters():
            if 'layer' in name and 'ner_model.bert' in name:
                gradient_mask[params] = params.new_zeros(params.size())
        
        train_batch_size = self.get_current_train_batch_size()
        self.pre_filling_config['use_pre_filling'] = 'ground_truth'
        self.pre_filling_config['ground_truth_pre_filling_ratio'] = 1.0
        iter_desc = 'Iteration'
        if self.in_distributed_mode():
            train_dataloader = self.prepare_dist_data_loader(
                self.train_dataset, train_batch_size, epoch=0
            )
        ...
        N = int(len(self.train_examples) / self.n_gpu)
        for step, batch in step_batch_iter:
            batch = self.set_batch_to_device(batch)

            # forward
            loss = model(
                batch, self.train_features, use_gold_span=True, train_flag=True, teacher_prob=1.0, pre_filling_config = self.pre_filling_config
            )
        ...
        for k, v in gradient_mask.items():
            ...
            if r is None:
                ...
                r = np.append(r, v)
        polar = np.percentile(r, (1-self.setting.reserve_p)*100)
    ...
    def base_train(self, get_loss_func, kwargs_dict1={},
        ...
        # =================== Set ChildTuning-D BEGIN =====================
        if self.setting.use_child_tune and self.setting.child_tune_mode == 'ChildTuning-D':
            self.logging('='*20 + 'Start Calculate Fisher' + '='*20)
            gradient_mask = self.calculate_fisher()
            self.logging('='*20 + 'Start Calculate Fisher' + '='*20)
            self.optimizer.set_gradient_mask(gradient_mask)  
        # =================== Set ChildTuning-D END =======================
        ...
        for epoch_idx in trange(base_epoch_idx, int(self.setting.num_train_epochs), desc="Epoch"):
            #pre-filling
            assert self.setting.start_ground_truth_pre_filling_epoch <= self.setting.end_ground_truth_pre_filling_epoch
            # just use for training, do not use in inference
            if self.setting.start_ground_truth_pre_filling_epoch <= epoch_idx < self.setting.end_ground_truth_pre_filling_epoch:
                self.pre_filling_config['use_pre_filling'] = 'ground_truth'
                step = (self.setting.max_ground_truth_pre_filling_ratio - self.setting.min_ground_truth_pre_filling_ratio) / (self.setting.end_ground_truth_pre_filling_epoch - self.setting.start_ground_truth_pre_filling_epoch)
                self.pre_filling_config['ground_truth_pre_filling_ratio'] = self.setting.max_ground_truth_pre_filling_ratio - (epoch_idx - self.setting.start_ground_truth_pre_filling_epoch) * step
                if self.pre_filling_config['ground_truth_pre_filling_ratio'] > 1:
                    self.pre_filling_config['ground_truth_pre_filling_ratio'] = 1.0
                if self.pre_filling_config['ground_truth_pre_filling_ratio'] < 0:
                    self.pre_filling_config['ground_truth_pre_filling_ratio'] = 0.0
            else:
                self.pre_filling_config['use_pre_filling'] = None
            ...
            batch_loss_list = []
            batch_total_grad_list = []
            batch_role_grad_list = []
            batch_global_step_list = []
            for step, batch in step_batch_iter:
                batch = self.set_batch_to_device(batch)
                ...
                batch_loss_list.append(loss_scalar)
                tr_loss += loss_scalar
                if self.is_master_node():
    ...
    def base_eval(self, eval_dataset, get_info_on_batch, reduce_info_type='mean', dump_pkl_path=None, **func_kwargs):
        ...
        for step, batch in enumerate(tqdm(eval_dataloader, desc=iter_desc)):
            ...
            # print('#xxx#---model output batch info')
            # print(batch_info)
        ...
        # print('#xxx#---reduced model out info')
        # print(reduced_info)
        return reduced_info
