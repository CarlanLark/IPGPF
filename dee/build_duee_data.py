"""
The codes are based on https://github.com/Spico197/DocEE
"""
...
from duee_wo_triggers import event_type2event_class
from itertools import chain

# def get_event_template(template_name):

#     template = importlib.import_module(f".{template_name}", "dee")
#     return template
...
def build(
    ...
    for d in load_line_json_iterator(filepath):
        ...
        tmp_ann_valid_dranges = list(chain.from_iterable(ann_valid_dranges))
        tmp_ann_valid_dranges = set([tuple(x) for x in tmp_ann_valid_dranges])
        tmp_ann_valid_dranges = [list(x) for x in tmp_ann_valid_dranges]
        ...
        data.append(
            [
                d["id"],
                {
                    "doc_type": doc_type,
                    "sentences": sents,
                    "ann_valid_mspans": list(set(ann_valid_mspans)),
                    "ann_valid_dranges": tmp_ann_valid_dranges, 
...
if __name__ == "__main__":
    build(
        event_type2event_class,
        "../Data/duee_fin_train.json",
        "../Data/dueefin_train_wo_tgg.json",
    )
    build(
        event_type2event_class, "../Data/duee_fin_dev.json", "../Data/dueefin_dev_wo_tgg.json"
    )
    build(
        event_type2event_class,
        "../Data/duee_fin_test2.json",
        "../Data/dueefin_submit_wo_tgg.json",
        inference=True,
    )