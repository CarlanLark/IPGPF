...
from .utils import default_load_json, default_dump_json, EPS, bert_chinese_char_tokenize
...
class NERExample(object):
    ...
    def get_char_entity_labels(self):
        ...
        while ent_idx < len(self.entity_range_span_types):
            ...
            if ent_cid_s > char_idx:
                ...
                ent_idx += 1 # old code cannot deal with the overlap entities
    ...
    def get_char_entity_labels_bieos(self):
        char_entity_labels = []
        char_idx = 0
        ent_idx = 0
        while ent_idx < len(self.entity_range_span_types):
            (ent_cid_s, ent_cid_e), ent_span, ent_type = self.entity_range_span_types[ent_idx]
            # Due to the overlap among Position, VictimPosition, and CriminalPosition, simply change them all to
            # Position.
            if ent_type in ["CriminalPosition", "VictimPosition"]:
                ent_type = "Position"
            # try:
            #     assert ent_cid_s < ent_cid_e <= self.num_chars
            # except:
            ...
            if ent_cid_s > char_idx:
                ...
                if ent_cid_e - ent_cid_s == 1:
                    # single token span.
                    tmp_ent_labels = ["S-" + ent_type]
                else:
                    tmp_ent_labels = ['B-' + ent_type] + ['I-' + ent_type] * (ent_cid_e - ent_cid_s - 2) + ['E-' + ent_type]
                char_entity_labels.extend(tmp_ent_labels)
                char_idx = ent_cid_e
                ent_idx += 1
            else:
...
class NERFeatureConverter(object):
    ...
    def convert_example_to_feature(self, ner_example, log_flag=False):
        ex_tokens = bert_chinese_char_tokenize(self.vocab, ner_example.text)
        #BIO format
