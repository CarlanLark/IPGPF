...
from .dee_metric import measure_event_table_filling, measure_ins_prf
...
from .utils import default_load_json, default_dump_json, default_dump_pkl, default_load_pkl, bert_chinese_char_tokenize, regex_extractor

from transformers import AutoTokenizer, BertTokenizer
import copy

from .event_type import local_type2global_type
...
class DEEExample(object):
    def __init__(self, annguid, detail_align_dict, only_inference=False, merge_entity = False):
        self.guid = annguid
        # [sent_text, ...]
        ...
        if only_inference:
            ...
            if merge_entity:
                ...
                for raw_field, ents in self.complementary_field2ents.items():
                    ...
                    for ent, pos_span in ents:
                        ...
                        if ent not in detail_align_dict["ann_valid_mspans"]:
                            ...
                            detail_align_dict["ann_mspan2guess_field"][ent] = raw_field
                ...
                mspan2dranges = copy.deepcopy(detail_align_dict["ann_mspan2dranges"])
                for ent, field in mspan2fields.items():
                    # 0. change local ment type to global ment type
                    if field in local_type2global_type:
                        detail_align_dict["ann_mspan2guess_field"][ent] = local_type2global_type[field]
                    # 1. change OtherType to the right type 
                    if field == "OtherType" and "%" in ent:
                        detail_align_dict["ann_mspan2guess_field"][ent] = "ratio"
                    if field == "OtherType" and "股" in ent:
                        detail_align_dict["ann_mspan2guess_field"][ent] = "share"
                    if field == "OtherType" and "元" in ent:
                        detail_align_dict["ann_mspan2guess_field"][ent] = "money"
                    if field == "OtherType" and "日" in ent:
                        detail_align_dict["ann_mspan2guess_field"][ent] = "date"
                    # 2. wrong ratio annotation correction
                    if field == "ratio":
                        for drange in mspan2dranges[ent]:
    ...
    @staticmethod
    def get_entity_label_list_bieos():
        visit_set = set()
        entity_label_list = [NERExample.basic_entity_label]
        ...
        for field in common_fields:
            if field not in visit_set:
                visit_set.add(field)
                entity_label_list.extend(['B-' + field, 'I-' + field, "E-" + field, "S-" + field])
...
class DEEExampleLoader(object):
    def __init__(self, rearrange_sent_flag, max_sent_len, merge_entity = False):
    ...
    def convert_dict_to_example(self, annguid, detail_align_info, only_inference=False):
        ...
        dee_example = DEEExample(annguid, detail_align_info, only_inference=only_inference, merge_entity = self.merge_entity)
    ...
    def __call__(self, dataset_json_path, only_inference = False):
        ...
        for annguid, detail_align_info in annguid_aligninfo_list:
            ...
            dee_example = self.convert_dict_to_example(annguid, detail_align_info, only_inference = only_inference)
...
class DEEFeature(object):
    ...
    def generate_label(self, pred_span_token_tup_list, return_miss=False):
        '''
...
class DEEFeatureConverter(object):
    def __init__(self, entity_label_list, event_type_fields_pairs,
                 max_sent_len, max_sent_num, tokenizer, template_text_type, 
        ...
        self.template_text_type = template_text_type
        ...
        self.template = Template(self.tokenizer, self.event_type_fields_pairs, self.template_text_type)
    ...
    def convert_example_to_feature(self, ex_idx, dee_example, log_flag=False):
        ...
        for mspan in dee_example.ann_valid_mspans:
            ...
            span_tokens = bert_chinese_char_tokenize(self.vocab, mspan)
            span_token_ids = tuple(self.tokenizer.convert_tokens_to_ids(span_tokens))
            # print('#xxx#')
            # print(tuple(self.tokenizer.convert_ids_to_tokens(span_token_ids)))
        ...
        for event_idx, event_type in enumerate(self.event_type_list):
            ...
            if event_type not in dee_example.event_type2event_objs:
                ...
                if event_arg_idxs_objs:
                    ...
                    event_arg_idxs_objs_list.append(None)
        # Templates
        template_info = self.template.template_info
        # entity_label2index
        # to avoid complex variable quoting, we plug entity_label2index in template
        # To do: add a dict named addition which contains all debug variables
        template_info['index2entity_label'] = {index: label for label, index in self.ner_fea_converter.entity_label2index.items()}
...
def measure_dee_prediction(event_type_fields_pairs, features, event_decode_results, index2entity_label, 
    ...
    for term in event_decode_results:
        ...
        # considering record polysemy within each gold_record_map
        for event_id, events in enumerate(gold_record_mat):
            if not events:
                continue
            event_role_list = event_type_fields_pairs[event_id][1]
            entity_dict = {}
            polysemy_list = []
            for event in events:
                for i, arg in enumerate(event):
                    if not arg:
                        continue
                    role = event_role_list[i]
                    if arg in entity_dict and entity_dict[arg] != role:
                        polysemy_list.append(arg)
                    else:
                        entity_dict[arg] = role
            polysemy_record_mat_list.append(polysemy_list)


    g_eval_res = measure_event_table_filling(
        pred_record_mat_list, gold_record_mat_list, event_type_fields_pairs, 
        pred_event_types, gold_event_types,
        pred_spans_token_tuple_list, gold_spans_token_tuple_list,
        dict_return=True, 
        polysemy_mat_list=polysemy_record_mat_list
    )
...
def measure_ins_prediction(event_type_fields_pairs, features, event_decode_results, 
                           dump_json_path=None):
    # measure prfs for each instance
    pred_event_types = []
    gold_event_types = []
    pred_spans_token_tuple_list = []
    gold_spans_token_tuple_list = []
    ...
    return ins_prf_stats

def print_ins_performance(mstr_bepoch_list, base_dir, features, 
                                      metric_type='micro', data_type='test', span_type='pred_span', 
                                      bert_model = None, dump_results = True):
    model_str2decode_results = {}
    for model_str, best_epoch in mstr_bepoch_list:
        model_str2decode_results[model_str] = resume_decode_results(
            base_dir, data_type, span_type, model_str, best_epoch
        ...
        event_type_fields_pairs = DEEExample.get_event_type_fields_pairs()
        ins_prf_stats = measure_ins_prediction(
                event_type_fields_pairs, features, total_decode_results, 
            )
        
        ins_dec_res = []
        for res in total_decode_results:
            ex_idx = res[0]
            pred_event_type = res[1]
            pred_event_records = res[2]
            event_dec_res = {}
            for idx, event in enumerate(pred_event_type):
                if event == 1:
                    records = pred_event_records[idx]
                    record_dec_res = []
                    if records != None:
                        for record in records:
                            ...
                            for tup in record:
                                if tup != None:
                                    text = tokenizer.convert_ids_to_tokens(tup)
                                    role_dec_res.append(''.join(text))
                                else:
                                    role_dec_res.append('空')
                            record_dec_res.append(role_dec_res)
                    else:
                        record_dec_res.append('无输出')
                    event_dec_res[event_type_fields_pairs[idx][0]] = record_dec_res
                else:
                    event_dec_res[event_type_fields_pairs[idx][0]] = []
            ins_dec_res.append(event_dec_res)
        
        assert len(ins_prf_stats) == len(ins_dec_res)
        final_res = []
        for event_idx, prf in enumerate(ins_prf_stats):
            temp = {'event_idx': event_idx,
                    'prf': prf, 
                    'decode_results': ins_dec_res[event_idx]
            }    
            final_res.append(temp)
        if dump_results:
            ins_decode_dump_template = 'dee_eval.{}.{}.{}.{}.{}.json'
            decode_fn = ins_decode_dump_template.format(data_type, span_type, model_str, best_epoch, 'ins')
            decode_fp = os.path.join(base_dir, decode_fn)
            logger.info('Dump instance decoded results to {}'.format(decode_fp))
            default_dump_json(final_res, decode_fp)