...
from .uie_model import UIE
import pdb

class BertForBasicNER(nn.Module):
    ...
    def __init__(self, config, num_entity_labels):
        super(BertForBasicNER, self).__init__()
        self.config = config
        if self.config.use_ernie:
            self.bert = UIE.from_pretrained(config.bert_model).encoder
        else:
            self.bert = AutoModel.from_pretrained(config.bert_model)

        self.dropout = nn.Dropout(config.dropout)
        self.classifier = nn.Linear(config.bert_size, num_entity_labels)
    ...
    def forward(self, input_ids, input_masks,
        ...
        batch_seq_enc = self.bert(input_ids, attention_mask=input_masks).last_hidden_state
